...
- 클러스터로 동작한다.
- 스케일 아웃이 잘되어 있음
  - 처음 만들어졌을때 부터 스케일 아웃을 잘할 수 있게 만들어짐
...
- RDB는 like할때 하나 하나씩 그것을 찾게 되는데
  - 처음부터 쭉 가게 되는데 full text를 잘 안함 데이터 많아지면 느려지기 때문에 하지만 elk에서는 full text를 사용
...
- 조인은 가능하긴 함
  - 스크립트 필드로 조인 같이 할 수 있지만 트릭임

- ELK같은 NoSql은 내가 보고 싶은 결과를 자체를 넣어놓고 빨리 찾을 수 있는것 을 넣어놓은것
  - 그래서 그것을 설계를 맞춰서 사용해야함
- RDB의 조인한 결과를 넣어서 써야하는 점이 있음,
  -  이것을 불편하다고 생각하면 안됨
